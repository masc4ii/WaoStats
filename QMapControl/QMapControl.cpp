...
namespace qmapcontrol
{
    ...
    void QMapControl::disablePersistentCache()
    {
        ImageManager::get().disablePersistentCache();
    }
    ...
    void QMapControl::paintEvent(QPaintEvent* paint_event)
    {
        ...
        if(m_scalebar_enabled)
        {
            ...
            if(m_current_zoom >= 0 && scalebar_distances.size() > m_current_zoom)
            {
                ...
                const double scalebar_line_length_px = devicePixelRatio() * scalebar_distances.at(m_current_zoom) / std::pow(2.0, 18 - m_current_zoom) / 0.597164;
